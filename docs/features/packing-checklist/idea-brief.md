...
# Idea Brief — Packing Checklist

<!-- Простий рівень ДЗ: інтерв'ю пройдено до кроку 2 (Сократ) включно, далі — Draft. -->

## 1. Raw idea
Чек-лист речей до поїздки прямо в trip-ledger, з багаторазовими шаблонами. Список у мене насправді вже є — у нотатках і месенджері, — але він не пов'язаний з поїздкою і губиться між поїздками. Критерій успіху: нуль речей, куплених повторно в поїздці через «забув».

## 2. Problem
Список зборів існує окремо від поїздки (нотатки/месенджер), щоразу шукається заново і не відтворюється між поїздками. Через це конкурентний аналіз цієї фічі має порівнювати з нотатками, а не з travel-застосунками. Метрика успіху — постфактум: кількість повторних покупок «бо забув» за поїздку → 0.
