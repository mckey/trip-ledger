...
4. **Модель PRD §9: курс 1 зберігається явно.** `AddExpense` пише `rate_nano = 10⁹` для витрат у base currency, міграція 0004 backfill-ить старі рядки, counted лишається «має збережений курс». Одне визначення для локу AC-07 і для підсумку, зате дублюється факт, який виводиться з двох наявних полів, а пізніше задання base currency поїздки потребує крос-BC запису у `expenses` (проставити 1 усім витратам у цій валюті) — саме та мутація «шаром знизу», якої Invariants велять уникати.

## Результат рішення

**Обрано:** Варіант 1 — єдина `BudgetBlock`, counted = ефективний курс, курс 1 похідний.

**Чому переміг:** єдиний варіант, у якому AC-09 виконується без другого агрегата, а AC-06 — без запису у БД: правило «валюта витрати = base currency ⇒ 1» — факт, який видно з двох уже наявних полів, тож зберігати його (і backfill-ити старі рядки, варіант 4) — дублювання. Побічно це знімає крос-BC запис, який інакше довелося б робити при пізнішому заданні base currency поїздки. Наслідок для локу AC-07 (ADR-0004): блокує лише **явний** rate snapshot; витрати з похідним курсом 1 після зміни валюти чесно стають «без курсу» і видні у лічильнику — це свідомо, а не пропущено.
...
**Негативні:**
- Словник перевизначається на двох рівнях: кореневий CONTEXT.md («converted total … лише з витрат, що мають rate snapshot») і `docs/features/trip-budget/CONTEXT.md` + SAD trip-budget §12 (counted = «у base currency») → `fix-term` на обидва до реалізації (sad.md §11).
...
- Інваріант словника: «будь-який перерахунок — окремий шар поверх, не мутація витрати» — захищає: курс 1 виводиться на читанні, у БД нічого не пишеться. Термін «converted total» у кореневому CONTEXT.md уточнюється цим ADR (fix-term, sad.md §11)
